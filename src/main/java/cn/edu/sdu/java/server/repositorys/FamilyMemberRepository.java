...
public interface FamilyMemberRepository extends JpaRepository<FamilyMember,Integer> {
    ...
    @Query("select member from FamilyMember member where member.student.person.personId = :personId")
    List<FamilyMember> findByStudentPersonId(@Param("personId") Integer personId);

    //增加验证信息
    @Query("select member from FamilyMember member where member.student.person.personId = :personId and member.memberId = :memberId")
    FamilyMember findByStudentPersonIdAndMemberId(@Param("personId") String personId, @Param("memberId") String memberId);

    //通过关系查找家庭成员
    @Query("select member from FamilyMember member where member.relation = :relation and member.student.person.personId = :personId")
